            ...
            Auth.auth().createUser(withEmail: email, password: password) { (user, error) in
                if error == nil {
                    ...
                    changeRequest?.commitChanges { (error) in
                        let alertController = UIAlertController(title: "Couldn't Update Username.", message: "Please Try Again Later", preferredStyle: .alert)
                        self.present(alertController, animated: true, completion: nil)
                    }
                    let alertController = UIAlertController(title: "Account has been successfully created", message:"Thanks", preferredStyle: .alert)
                    let segueAction = UIAlertAction(title:"Thx", style: .default, handler: {(action) -> Void in
