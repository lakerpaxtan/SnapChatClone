            ...
            if let posts = snapshot.value as? [String:AnyObject] {
                user.getReadPostIDs(completion: { (ids) in
                    for postKey in posts.keys {
                        if let tempDict = posts[postKey] as? [String:String]{
                            if let tempUserName = tempDict["username"]{
                                if let tempThread = tempDict["thread"]{
                                    if let tempDate = tempDict["date"]{
                                        if let tempImagePath = tempDict["imagePath"]{
                                            let hasRead = ids.contains(postKey)
                                            let newPost = Post(id: postKey, username: tempUserName , postImagePath: tempImagePath , thread: tempThread , dateString: tempDate , read: hasRead)
